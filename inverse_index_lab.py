...
def makeInverseIndex(strlist):
    ...
    True
    """
    strlist = [s.split() for s in strlist]
    words = {word for s in strlist for word in s}
    return (
        {word: {doc_id for doc_id, doc_str in enumerate(strlist)
        if word in doc_str}
        for word in words})
...
def orSearch(inverseIndex, query):
    ...
    """
    res = set()
    res.update(*[inverseIndex[word] for word in query])
    return res
...
def andSearch(inverseIndex, query):
    ...
    True
    """
    res = set(inverseIndex[query[0]])
    res.intersection_update(*[inverseIndex[word] for word in query[1:]])
    return res