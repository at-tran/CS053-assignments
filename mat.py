...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return False not in [A[k0, k1] == B[k0, k1] for k0 in A.D[0] for k1 in A.D[1]]
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    ks = set(A.f.keys()) | set(B.f.keys())
    return Mat(A.D, {k: A[k] + B[k] for k in ks})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {k: M[k] * x for k in M.f.keys()})
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {c: v * Vec(M.D[0], {r: M[r,c]for r in M.D[0]}) for c in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], {r: Vec(M.D[1], {c: M[r, c] for c in M.D[1]}) * v for r in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    return Mat((A.D[0], B.D[1]), \
               {(r, c): Vec(A.D[1], {x: A[r, x] for x in A.D[1]}) \
                      * Vec(B.D[0], {x: B[x, c] for x in B.D[0]}) \
