...
from Dimension_other_problems import subset_basis
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    Example:
        ...
        True
    '''
    sol = vec2rep(U_basis + V_basis, w)
    U_D = range(len(U_basis))
    V_D = range(len(V_basis))
    return (coldict2mat(U_basis) * Vec(set(U_D), {x: sol[x] for x in U_D}),
            coldict2mat(V_basis) * Vec(set(V_D), {x: sol[x + len(U_basis)] for x in V_D}))
