...
def make_nine_equations(corners):
    ...
    ''' 
    return [row for ((x1, x2), (w1, w2)) in (zip(corners,[(0, 0), (0, 1), (1, 0), (1, 1)])) for row in make_equations(x1, x2, w1, w2)] + [w]
...
H = Mat(({'y1', 'y2', 'y3'}, {'x1', 'x2', 'x3'}), {(yi, xi): hvec[yi, xi] for yi in ['y1', 'y2', 'y3'] for xi in ['x1', 'x2', 'x3']})
...
def mat_move2board(Y):
    ...
    <BLANKLINE>
    '''
    return Mat(Y.D, {(yi, i): Y[yi, i] / Y['y3', i] for i in Y.D[1] for yi in Y.D[0]})



# This is my own code, not part of lab
def transfrom_perspective(file, corners):
    hvec = solve(rowdict2mat(make_nine_equations(corners)), b)
    H = Mat(({'y1', 'y2', 'y3'}, {'x1', 'x2', 'x3'}), {(yi, xi): hvec[yi, xi] for yi in ['y1', 'y2', 'y3'] for xi in ['x1', 'x2', 'x3']})
    
    X_pts, colors = file2mat(file, ('x1', 'x2', 'x3'))
    Y_pts = H * X_pts
